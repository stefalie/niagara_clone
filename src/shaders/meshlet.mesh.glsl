...
layout(binding = 0) readonly buffer Vertices
{
	...
	// Minecraft/Roblox is crazy -> 41 triangles

#define USE_PACKED_INDICES 0

struct Meshlet
{
	uint vertices[64];
#if !USE_PACKED_INDICES
	uint8_t indices[126 * 3];  // Max 126 triangles.
	uint8_t pad_1;
	uint8_t pad_2;
#else  // This will completely break the normal computation.
	uint indices[95];  // Max 126 triangles.
#endif
...
#else
	for (uint i = ti; i < index_count; i += 128)
	{
		writePackedPrimitiveIndices4x8NV(i * 4, i);
	}
#endif
